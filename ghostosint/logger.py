...
from ghostosint import GhostOsintDB, GhostOsintHelp


class GhostOsintSqlLogHandler(logging.Handler):
    ...
    def makeDbh(self) -> None:
        """TBD."""
        self.dbh = GhostOsintDB(self.opts)


def logListenerSetup(loggingQueue, opts: dict = None) -> 'logging.handlers.QueueListener':
    """Create and start a GhostOSINT log listener in its own thread.
    ...
    Args:
        ...
        opts (dict): GhostOSINT config

    Returns:
        GhostOsintLogListen (logging.handlers.QueueListener): Log listener
    """
    if opts is None:
        opts = dict()
    ...
    log_dir = GhostOsintHelp.logPath()
    debug_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/ghostosint.debug.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    error_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/ghostosint.error.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    if doLogging and opts is not None:
        sqlite_handler = GhostOsintSqlLogHandler(opts)
        sqlite_handler.setLevel(logLevel)
        sqlite_handler.setFormatter(log_format)
        handlers.append(sqlite_handler)
    GhostOsintLogListen = QueueListener(loggingQueue, *handlers)
    GhostOsintLogListen.start()
    atexit.register(stop_listener, GhostOsintLogListen)
    return GhostOsintLogListen


def logWorkerSetup(loggingQueue) -> 'logging.Logger':
    """Root GhostOSINT logger.
    ...
    """
    log = logging.getLogger("ghostosint")
