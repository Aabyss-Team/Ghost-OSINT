...
class GhostOsintEvent():
    """GhostOsintEvent object representing identified data and associated meta data.
    ...
    Attributes:
        ...
        sourceEvent (GhostOsintEvent): GhostOsintEvent that triggered this event
        sourceEventHash (str): Hash of the GhostOsintEvent event that triggered this event
    ...
    def __init__(self, eventType: str, data: str, module: str, sourceEvent: 'GhostOsintEvent') -> None:
        """Initialize GhostOSINT event object.

        Args:
            eventType (str): Event type, e.g. URL_FORM, RAW_DATA, etc.
            data (str): Event data, e.g. a URL, port number, webpage content, etc.
            module (str): Module from which the event originated
            sourceEvent (GhostOsintEvent): GhostOsintEvent event that triggered this event
        """
    ...
    @property
    def sourceEvent(self) -> 'GhostOsintEvent':
    ...
    @sourceEvent.setter
    def sourceEvent(self, sourceEvent: 'GhostOsintEvent') -> None:
        """Source event which lead to this event.

        Args:
            sourceEvent (GhostOsintEvent): source event
        ...
        if not isinstance(sourceEvent, GhostOsintEvent):
            raise TypeError(f"sourceEvent is {type(sourceEvent)}; expected GhostOsintEvent()")
