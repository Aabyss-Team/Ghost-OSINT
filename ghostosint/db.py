...
class GhostOsintDB:
    ...
    # Queries for creating the GhostOSINT database
    createSchemaQueries = [
    ...
    eventDetails = [
        ['ROOT', 'Internal GhostOSINT Root event', 1, 'INTERNAL'],
        ...
        with self.dbhLock:
            try:
                ...
                try:
                    ...
                    raise IOError(f"Tried to set up the GhostOSINT database schema, but failed: {e.args[0]}")
    ...
    def scanEventStore(self, instanceId, GOEvent, truncateSize=0):
        """Store an event in the database.

        Args:
            instanceId (str): scan instance ID
            GOEvent (GhostOsintEvent): event to be stored in the database
        ...
        if not isinstance(GOEvent.generated, float):
            raise TypeError(f"GOEvent.generated is {type(GOEvent.generated)}; expected float()")

        if not GOEvent.generated:
            raise ValueError("GOEvent.generated is empty")

        if not isinstance(GOEvent.eventType, str):
            raise TypeError(f"GOEvent.eventType is {type(GOEvent.eventType,)}; expected str()")

        if not GOEvent.eventType:
            raise ValueError("GOEvent.eventType is empty")

        if not isinstance(GOEvent.data, str):
            raise TypeError(f"GOEvent.data is {type(GOEvent.data)}; expected str()")

        if not GOEvent.data:
            raise ValueError("GOEvent.data is empty")

        if not isinstance(GOEvent.module, str):
            raise TypeError(f"GOEvent.module is {type(GOEvent.module)}; expected str()")

        if not GOEvent.module:
            if GOEvent.eventType != "ROOT":
                raise ValueError("GOEvent.module is empty")

        if not isinstance(GOEvent.confidence, int):
            raise TypeError(f"GOEvent.confidence is {type(GOEvent.confidence)}; expected int()")

        if not 0 <= GOEvent.confidence <= 100:
            raise ValueError(f"GOEvent.confidence value is {type(GOEvent.confidence)}; expected 0 - 100")

        if not isinstance(GOEvent.visibility, int):
            raise TypeError(f"GOEvent.visibility is {type(GOEvent.visibility)}; expected int()")

        if not 0 <= GOEvent.visibility <= 100:
            raise ValueError(f"GOEvent.visibility value is {type(GOEvent.visibility)}; expected 0 - 100")

        if not isinstance(GOEvent.risk, int):
            raise TypeError(f"GOEvent.risk is {type(GOEvent.risk)}; expected int()")

        if not 0 <= GOEvent.risk <= 100:
            raise ValueError(f"GOEvent.risk value is {type(GOEvent.risk)}; expected 0 - 100")

        if not isinstance(GOEvent.sourceEvent, GhostOsintEvent):
            if GOEvent.eventType != "ROOT":
                raise TypeError(f"GOEvent.sourceEvent is {type(GOEvent.sourceEvent)}; expected str()")

        if not isinstance(GOEvent.sourceEventHash, str):
            raise TypeError(f"GOEvent.sourceEventHash is {type(GOEvent.sourceEventHash)}; expected str()")

        if not GOEvent.sourceEventHash:
            raise ValueError("GOEvent.sourceEventHash is empty")

        storeData = GOEvent.data
        ...
        qvals = [instanceId, GOEvent.hash, GOEvent.eventType, GOEvent.generated,
                 GOEvent.confidence, GOEvent.visibility, GOEvent.risk,
                 GOEvent.module, storeData, GOEvent.sourceEventHash]
