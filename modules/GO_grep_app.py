...
class GO_grep_app(GhostOsintPlugin):
    ...
    def query(self, qry, page):
        ...
        res = self.GhostOsint.fetchUrl("https://grep.app/api/search?" + urllib.parse.urlencode(params),
    ...
    def handleEvent(self, event):
        ...
        while page <= pages:
            ...
            for result in data:
                ...
                links = self.GhostOsint.extractUrls(snippet.replace('<mark>', '').replace('</mark>', ''))
                if links:
                    for link in links:
                        ...
                        if not self.getTarget().matches(self.GhostOsint.urlFQDN(link), includeChildren=True, includeParents=True):
                ...
                emails = self.GhostOsint.parseEmails(snippet.replace('<mark>', '').replace('</mark>', ''))
                if emails:
                    for email in emails:
