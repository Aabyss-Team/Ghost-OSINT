...
class GO_cloudflaredns(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        malware = self.GhostOsint.normalizeDNS(self.queryMalwareDNS(eventData))
        ...
        if '0.0.0.0' not in malware:
            ...
            evt = GhostOsintEvent(e, f"CloudFlare - Family [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)
            return

        # Host is blocked only by malware filters
        self.debug(f"{eventData} blocked by CloudFlare Malware DNS")
        evt = GhostOsintEvent(e, f"CloudFlare - Malware [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)
