...
class GO_haveibeenpwned(GhostOsintPlugin):
    ...
    def query(self, qry):
        ...
        while retry < 2:
            ...
            res = self.GhostOsint.fetchUrl(url, timeout=self.opts['_fetchtimeout'],
                                   useragent="GhostOSINT", headers=hdrs)
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled GO_haveibeenpwned but did not set an API key!")
            self.errorState = True
            return
        ...
        if data is not None:
            for n in data:
                ...
                if eventName == 'EMAILADDR':
                    e = GhostOsintEvent("EMAILADDR_COMPROMISED", eventData + " [" + site + "]",
                                        self.__name__, event)
                else:
                    e = GhostOsintEvent("PHONE_NUMBER_COMPROMISED", eventData + " [" + site + "]",
                                        self.__name__, event)
        ...
        for link in links:
            try:
                ...
                res = self.GhostOsint.fetchUrl(link, timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
...
# End of GO_haveibeenpwned class
