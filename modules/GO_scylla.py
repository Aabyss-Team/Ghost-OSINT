...
class GO_scylla(GhostOsintPlugin):
    ...
    def query(self, qry, per_page=20, start=0):
        ...
        res = self.GhostOsint.fetchUrl(f"https://scylla.so/search?{urllib.parse.urlencode(params)}",
                               headers=headers,
                               timeout=15,
    ...
    def handleEvent(self, event):
        ...
        for pass_hash in set(hashes):
            evt = GhostOsintEvent('HASH_COMPROMISED', pass_hash, self.__name__, event)
            self.notifyListeners(evt)

        for email in set(emails):
            evt = GhostOsintEvent('EMAILADDR_COMPROMISED', email, self.__name__, event)
            self.notifyListeners(evt)

        for password in set(passwords):
            evt = GhostOsintEvent('PASSWORD_COMPROMISED', password, self.__name__, event)
            self.notifyListeners(evt)

# End of GO_scylla class
