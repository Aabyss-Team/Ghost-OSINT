...
#               modules (such as GO_spider) and building up information about
...
class GO_pageinfo(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if "GO_spider" not in event.module:
        ...
        for regexpGrp in regexps:
            ...
            for regex in regexps[regexpGrp]:
                ...
                if len(matches) > 0 and regexpGrp not in self.results[eventSource]:
                    ...
                    evt = GhostOsintEvent(regexpGrp, eventSource, self.__name__, event)
        ...
        if len(self.results[eventSource]) == 0:
            ...
            evt = GhostOsintEvent("URL_STATIC", eventSource, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if len(matches) > 0:
            for match in matches:
                if '://' not in match:
                    continue
                if not self.GhostOsint.urlFQDN(match):
                    continue
                if self.getTarget().matches(self.GhostOsint.urlFQDN(match)):
                    continue
                self.debug(f"Externally hosted JavaScript found at: {match}")
                evt = GhostOsintEvent("PROVIDER_JAVASCRIPT", match, self.__name__, event)
                self.notifyListeners(evt)

# End of GO_pageinfo class
