...
# -------------------------------------------------------------------------------
# Name:         GO_dnsresolve
# Purpose:      GhostOSINT plug-in for extracting hostnames from identified data
#               and resolving them.
#
...
class GO_dnsresolve(GhostOsintPlugin):
    ...
    def enrichTarget(self, target):
        ...
        for host in ret:
            ...
            if self.GhostOsint.validIP(host):
                target.setAlias(host, "IP_ADDRESS")
            elif self.GhostOsint.validIP6(host):
                ...
                #     dom = self.GhostOsint.hostDomain(host, self.opts['_internettlds'])
    ...
    def resolveTargets(self, target, validateReverse: bool) -> list:
        ...
        Args:
            target (GhostOsintTarget): target object
        ...
        if t in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            r = self.GhostOsint.resolveIP(v)
            if r:
                ret.extend(r)
        if t == "INTERNET_NAME":
            r = self.GhostOsint.resolveHost(v)
            if r:
                ret.extend(r)
            r = self.GhostOsint.resolveHost6(v)
            if r:
                ret.extend(r)
        if t == "NETBLOCK_OWNER":
            ...
            for addr in IPNetwork(v):
                ...
                names = self.GhostOsint.resolveIP(ipaddr)
                if not names:
                    continue
                ...
                for host in names:
                    chk = self.GhostOsint.resolveHost(host)
        ...
        if t == "NETBLOCKV6_OWNER":
            ...
            for addr in IPNetwork(v):
                ...
                for host in names:
                    chk = self.GhostOsint.resolveHost6(host)
                    if chk and ipaddr in chk:
                        ret.append(host)
    ...
    def handleEvent(self, event) -> None:
        ...
        if srcModuleName in ["GO_dnsresolve"] and "_NAME" in eventName:
            return
        ...
        if eventData.endswith(".ipv6-literal.net") and eventName == "AFFILIATE_INTERNET_NAME":
            ...
            if self.GhostOsint.validIP6(ipv6):
                if self.getTarget().matches(ipv6):
                    evt = GhostOsintEvent("IPV6_ADDRESS", ipv6, self.__name__, parentEvent)
                else:
                    evt = GhostOsintEvent("AFFILIATE_IPV6_ADDRESS", ipv6, self.__name__, parentEvent)
        ...
        if eventData.endswith(".in-addr.arpa") and eventName == "AFFILIATE_INTERNET_NAME":
            ...
            if self.GhostOsint.validIP(ipv4):
                if self.getTarget().matches(ipv4):
                    evt = GhostOsintEvent("IP_ADDRESS", ipv4, self.__name__, parentEvent)
                else:
                    evt = GhostOsintEvent("AFFILIATE_IPADDR", ipv4, self.__name__, parentEvent)
                self.notifyListeners(evt)
        ...
        if eventName in ["CO_HOSTED_SITE", "AFFILIATE_INTERNET_NAME"]:
            ...
            if self.GhostOsint.isDomain(eventData, self.opts['_internettlds']):
                evt = GhostOsintEvent(ev, eventData, self.__name__, parentEvent)
                self.notifyListeners(evt)

            # In case the domain of the provided host is different, report that too
            dom = self.GhostOsint.hostDomain(eventData, self.opts['_internettlds'])
            if dom and dom != eventData:
                evt = GhostOsintEvent(ev, dom, self.__name__, parentEvent)
                self.notifyListeners(evt)

        # Resolve host names
        if eventName in ["INTERNET_NAME", "AFFILIATE_INTERNET_NAME"]:
            addrs = list()
            addrs.extend(self.GhostOsint.resolveHost(eventData))
            addrs.extend(self.GhostOsint.resolveHost6(eventData))
        ...
        # Reverse resolve IP addresses
            ...
            addrs = self.GhostOsint.resolveIP(eventData)
            ...
            for ip in IPNetwork(eventData):
                ...
                if self.GhostOsint.validIP(ipaddr):
                    if ipaddr.split(".")[3] in ['255', '0']:
                        continue
                    if '255' in ipaddr.split("."):
                        continue

                addrs = self.GhostOsint.resolveIP(ipaddr)
                if not addrs:
                    continue
    ...
    def processHost(self, host, parentEvent, affiliate=None) -> None:
        ...
        if affil:
            if self.GhostOsint.isValidLocalOrLoopbackIp(host):
                htype = "INTERNAL_IP_ADDRESS"
            elif self.GhostOsint.validIP(host):
        ...
        if htype in ["INTERNET_NAME", "AFFILIATE_INTERNET_NAME"]:
            if not self.GhostOsint.resolveHost(host) and not self.GhostOsint.resolveHost6(host):
                evt = GhostOsintEvent(f"{htype}_UNRESOLVED", host, self.__name__, parentEvent)
                self.notifyListeners(evt)
                return
        ...
        if htype == "INTERNET_NAME":
            ...
            ip6s = self.GhostOsint.resolveHost6(host)
            if not ip6s:
                return
            for ip6 in ip6s:
                parentHash = self.GhostOsint.hashstring(evt.data)
                ...
                evt6 = GhostOsintEvent("IPV6_ADDRESS", ip6, self.__name__, evt)
        ...
        if htype == "AFFILIATE_INTERNET_NAME":
            ...
            if not dom:
                return
            if dom == host and not self.GhostOsint.isDomain(dom, self.opts['_internettlds']):
                return
    ...
    def processDomain(self, domainName, parentEvent, affil=False, host=None) -> None:
        ...
        if affil:
            domevt = GhostOsintEvent("AFFILIATE_DOMAIN_NAME", domainName,
                                     self.__name__, parentEvent)
            self.notifyListeners(domevt)
            return

        if self.getTarget().matches(domainName):
            domevt = GhostOsintEvent("DOMAIN_NAME", domainName,
                                     self.__name__, parentEvent)
            self.notifyListeners(domevt)
        else:
            ...
            if parentEvent.data.endswith("." + domainName):
                domevt = GhostOsintEvent("DOMAIN_NAME_PARENT", domainName,
                                         self.__name__, parentEvent)
                self.notifyListeners(domevt)

# End of GO_dnsresolve class
