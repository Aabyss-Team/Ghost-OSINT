# -------------------------------------------------------------------------------
# Name:        GO_callername
# Purpose:     GhostOSINT plug-in to search CallerName.com for a phone number
...
class GO_callername(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        res = self.GhostOsint.fetchUrl(url, timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
        ...
        if rep_good_match and rep_bad_match:
            ...
            if bad_votes > good_votes:
                ...
                evt = GhostOsintEvent('MALICIOUS_PHONE_NUMBER', text, self.__name__, event)
                self.notifyListeners(evt)

# End of GO_callername class
