...
# -------------------------------------------------------------------------------
# Name:         GO_similar
# Purpose:      GhostOSINT plug-in for identifying domains that look similar
...
class GO_similar(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        dom = self.GhostOsint.domainKeyword(eventData, self.opts['_internettlds'])
        if not dom:
            return
            ...
            try:
                for domain in [f"{d}{tld}", f"www.{d}{tld}"]:
                    if self.GhostOsint.resolveHost(domain) or self.GhostOsint.resolveHost6(domain):
                        ...
                        evt = GhostOsintEvent("SIMILARDOMAIN", f"{d}{tld}", self.__name__, event)
                        self.notifyListeners(evt)
                        break
            except Exception:
                continue

# End of GO_similar class
