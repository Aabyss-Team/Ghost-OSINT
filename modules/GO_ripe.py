...
class GO_ripe(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "BGP_AS_OWNER":
            ...
            for netblock in netblocks:
                ...
                if ":" in netblock:
                    evt = GhostOsintEvent("NETBLOCKV6_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = GhostOsintEvent("NETBLOCK_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)

            evt = GhostOsintEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if eventName.startswith("NETBLOCK"):
            ...
            if eventName in ["NETBLOCK_OWNER", "NETBLOCKV6_OWNER"] and self.ownsAs(asn):
                asevt = GhostOsintEvent("BGP_AS_OWNER", asn, self.__name__, event)
                self.notifyListeners(asevt)
                evt = GhostOsintEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
                self.notifyListeners(evt)
            else:
                asevt = GhostOsintEvent("BGP_AS_MEMBER", asn, self.__name__, event)
                self.notifyListeners(asevt)
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            if not self.GhostOsint.validIpNetwork(prefix):
                return
            ...
            if ":" in prefix:
                evt = GhostOsintEvent("NETBLOCKV6_" + relationship, prefix, self.__name__, event)
            else:
                evt = GhostOsintEvent("NETBLOCK_" + relationship, prefix, self.__name__, event)
            self.notifyListeners(evt)

# End of GO_ripe class
