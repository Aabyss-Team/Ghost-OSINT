...
class GO_comodo(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if not self.GhostOsint.resolveHost(eventData) and not self.GhostOsint.resolveHost6(eventData):
            return
        ...
        evt = GhostOsintEvent(blacklist_type, f"Comodo Secure DNS [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)

        evt = GhostOsintEvent(malicious_type, f"Comodo Secure DNS [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)

# End of GO_comodo class
