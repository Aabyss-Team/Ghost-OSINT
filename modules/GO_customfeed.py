...
class GO_customfeed(GhostOsintPlugin):
    ...
    def resourceList(self, replaceme_id, target, targetType):
        ...
        if targetType == "domain":
            targetDom = self.GhostOsint.hostDomain(target, self.opts['_internettlds'])
            if not targetDom:
                return None
    ...
    def handleEvent(self, event):
        ...
        if self.opts['url'] == "":
            self.error("You enabled GO_customfeed but defined no custom feed URL!")
