...
class GO_dronebl(GhostOsintPlugin):

    meta = {
        'name': "DroneBL",
        'summary': "查询 DroneBL 数据库中的开放中继、开放代理、易受攻击的服务器等等内容.",
        'flags': [],
        ...
        'dataSource': {
            ...
            'description': "DroneBL 是滥用IP地址的实时监视器，其目标是阻止对受感染机器的滥用.\n"
            "滥用IP地址的实时追踪器！！",
        }
    }
        ...
        for addr in addrs:
            ...
            for result in res:
                ...
                if k in ["127.0.0.8", "127.0.0.9", "127.0.0.10", "127.0.0.11", "127.0.0.14"]:
                    evt = GhostOsintEvent("PROXY_HOST", addr, self.__name__, event)
                    self.notifyListeners(evt)

                if k == "127.0.0.19":
                    evt = GhostOsintEvent("VPN_HOST", addr, self.__name__, event)
                    self.notifyListeners(evt)

# End of GO_dronebl class
