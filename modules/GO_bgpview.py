...
class GO_bgpview(GhostOsintPlugin):
    ...
    def queryAsn(self, qry):
        res = self.GhostOsint.fetchUrl("https://api.bgpview.io/asn/" + qry.replace('AS', ''),
    ...
    def queryIp(self, qry):
        res = self.GhostOsint.fetchUrl("https://api.bgpview.io/ip/" + qry,
                               useragent=self.opts['_useragent'],
                               timeout=self.opts['_fetchtimeout'])
    ...
    def queryNetblock(self, qry):
        res = self.GhostOsint.fetchUrl("https://api.bgpview.io/prefix/" + qry,
                               useragent=self.opts['_useragent'],
                               timeout=self.opts['_fetchtimeout'])
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'BGP_AS_MEMBER':
            ...
            evt = GhostOsintEvent('PHYSICAL_ADDRESS', ', '.join([_f for _f in address if _f]), self.__name__, event)
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            for prefix in prefixes:
                ...
                evt = GhostOsintEvent("BGP_AS_MEMBER", str(asn), self.__name__, event)
                self.notifyListeners(evt)

                if self.GhostOsint.validIpNetwork(p):
                    if ":" in p:
                        evt = GhostOsintEvent("NETBLOCKV6_MEMBER", p, self.__name__, event)
                    else:
                        evt = GhostOsintEvent("NETBLOCK_MEMBER", p, self.__name__, event)
                    self.notifyListeners(evt)

# End of GO_bgpview class
