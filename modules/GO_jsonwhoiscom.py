...
class GO_jsonwhoiscom(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled GO_jsonwhoiscom but did not set an API key!")
            self.errorState = True
            return
        ...
        for contact in contacts:
            ...
            country = self.GhostOsint.countryNameFromCountryCode(contact.get('country_code'))
        ...
        for email in set(emails):
            ...
            if self.getTarget().matches(mail_domain, includeChildren=True):
                ...
                evt = GhostOsintEvent("AFFILIATE_EMAILADDR", email, self.__name__, event)
                self.notifyListeners(evt)

        if eventName in ["DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = GhostOsintEvent("DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)
            ...
            for dns_provider in set(dns_providers):
                evt = GhostOsintEvent("PROVIDER_DNS", dns_provider, self.__name__, event)
                self.notifyListeners(evt)

            for name in set(names):
                evt = GhostOsintEvent("RAW_RIR_DATA", f"Possible full name {name}", self.__name__, event)
                self.notifyListeners(evt)

            for phone in set(phones):
                evt = GhostOsintEvent("PHONE_NUMBER", phone, self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName in ["AFFILIATE_DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = GhostOsintEvent("AFFILIATE_DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)

            available = res.get('available?')
            if available:
                evt = GhostOsintEvent("AFFILIATE_DOMAIN_UNREGISTERED", eventData, self.__name__, event)
                self.notifyListeners(evt)

# End of GO_jsonwhoiscom class
