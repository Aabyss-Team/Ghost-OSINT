...
class GO_onioncity(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled GO_onioncity but did not set a Google API key!")
        ...
        response = self.GhostOsint.fetchUrl(
            googlesearch_url,
            timeout=self.opts["_fetchtimeout"],
            useragent=self.opts["_useragent"],
        )
        ...
        darknet_links = [
            link for link in new_links if self.GhostOsint.urlFQDN(link).endswith(".onion.link")
        ]
        ...
        for link in darknet_links:
            ...
            if self.opts['fetchlinks']:
                res = self.GhostOsint.fetchUrl(torlink, timeout=self.opts['_fetchtimeout'],
                                       useragent=self.opts['_useragent'],
                                       verify=False)
                ...
                evt = GhostOsintEvent("DARKNET_MENTION_URL", torlink, self.__name__, event)
                self.notifyListeners(evt)
