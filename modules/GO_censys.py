...
class GO_censys(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            e = GhostOsintEvent("RAW_RIR_DATA", json.dumps(rec), self.__name__, pevent)
            ...
            try:
                ...
                if location:
                    ...
                    if geoinfo:
                        e = GhostOsintEvent("GEOINFO", geoinfo, self.__name__, pevent)
                        self.notifyListeners(e)
            except Exception as e:
            ...
            try:
                ...
                if services:
                    for service in services:
                        ...
                        if port:
                            evt = GhostOsintEvent("TCP_PORT_OPEN", f"{addr}:{port}", self.__name__, pevent)
                        ...
                        if headers:
                            ...
                            e = GhostOsintEvent("WEBSERVER_HTTPHEADERS", dat, self.__name__, pevent)
                            e.actualSource = addr
                            self.notifyListeners(e)
            except Exception as e:
            ...
            try:
                ...
                if autonomous_system:
                    ...
                    if asn:
                        e = GhostOsintEvent("BGP_AS_MEMBER", str(asn), self.__name__, pevent)
                        self.notifyListeners(e)

                    bgp_prefix = autonomous_system.get('bgp_prefix')
                    if bgp_prefix:
                        e = GhostOsintEvent("NETBLOCK_MEMBER", str(bgp_prefix), self.__name__, pevent)
                        self.notifyListeners(e)
            except Exception as e:
