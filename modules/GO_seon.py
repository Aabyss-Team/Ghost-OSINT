...
# -------------------------------------------------------------------------------
# Name:         GO_seon
# Purpose:      ghostosint plugin to query seon.io to gather intelligence about
...
class GO_seon(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled GO_seon but did not set an API key!")
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            if resultSet:
                if resultSet.get('score', 0) >= self.opts['fraud_threshold']:
                    ...
                    if resultSet.get('tor'):
                        evt = GhostOsintEvent("WEBSERVER_TECHNOLOGY", f"Server is TOR node: {resultSet.get('tor')}", self.__name__, event)
                        self.notifyListeners(evt)
                    ...
                    if resultSet.get('vpn'):
                        evt = GhostOsintEvent("WEBSERVER_TECHNOLOGY", f"Server is VPN: {resultSet.get('vpn')}", self.__name__, event)
                        self.notifyListeners(evt)
                    ...
                    if resultSet.get('web_proxy'):
                        evt = GhostOsintEvent("WEBSERVER_TECHNOLOGY", f"Server is Web Proxy: {resultSet.get('web_proxy')}", self.__name__, event)
                        self.notifyListeners(evt)
                    ...
                    if resultSet.get('public_proxy'):
                        evt = GhostOsintEvent("WEBSERVER_TECHNOLOGY", f"Server is Public Proxy: {resultSet.get('public_proxy')}", self.__name__, event)
                        self.notifyListeners(evt)
                ...
                if resultSet.get('country'):
                    ...
                    evt = GhostOsintEvent('PHYSICAL_COORDINATES', f"{resultSet.get('latitude')}, {resultSet.get('longitude')}", self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                if resultSet.get('open_ports'):
                    for port in resultSet.get('open_ports'):
                        evt = GhostOsintEvent('TCP_PORT_OPEN', f"{eventData}:{port}", self.__name__, event)
                        self.notifyListeners(evt)
                        dataFound = True
            ...
            if resultSet:
                ...
                if resultSet.get('deliverable'):
                    evt = GhostOsintEvent("EMAILADDR_DELIVERABLE", eventData, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True
                else:
                    evt = GhostOsintEvent("EMAILADDR_UNDELIVERABLE", eventData, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True
                    ...
                    for site in socialMediaList:
                        if resultSet.get('account_details').get(site):
                            if resultSet.get('account_details').get(site).get('url'):
                                evt = GhostOsintEvent("SOCIAL_MEDIA", f"{site}: <SFURL>{resultSet.get('account_details').get(site).get('url')}</SFURL>", self.__name__, event)
                                self.notifyListeners(evt)
                            elif resultSet.get('account_details').get(site).get('registered'):
                                evt = GhostOsintEvent("SOCIAL_MEDIA", f"Registered on {site}", self.__name__, event)
                                self.notifyListeners(evt)
                            dataFound = True

                            if site == 'linkedin':
                                if resultSet.get('account_details').get(site).get('company'):
                                    evt = GhostOsintEvent("COMPANY_NAME", resultSet.get('account_details').get(site).get('company'), self.__name__, event)
                                    self.notifyListeners(evt)
                                    dataFound = True

                                if resultSet.get('account_details').get(site).get('name'):
                                    evt = GhostOsintEvent("HUMAN_NAME", resultSet.get('account_details').get(site).get('name'), self.__name__, event)
                                    self.notifyListeners(evt)
                                    dataFound = True
                ...
                if resultSet.get('breach_details').get('breaches'):
                    ...
                    for breachSet in breachList:
                        evt = GhostOsintEvent("EMAILADDR_COMPROMISED", f"{eventData} [{breachSet.get('name', 'Unknown')}]", self.__name__, event)
                        self.notifyListeners(evt)
                        dataFound = True
            ...
            if resultSet:
                ...
                if resultSet.get('type'):
                    evt = GhostOsintEvent("PHONE_NUMBER_TYPE", resultSet.get('type'), self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                if resultSet.get('carrier'):
                    evt = GhostOsintEvent("PROVIDER_TELCO", resultSet.get('carrier'), self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True
