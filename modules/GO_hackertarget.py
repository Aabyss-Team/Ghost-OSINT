# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         GO_hackertarget
# Purpose:      GhostOSINT plug-in to search HackerTarget.com for hosts sharing
...
class GO_hackertarget(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "GO_hackertarget" and eventName == "IP_ADDRESS":
        ...
        for ip in qrylist:
                ...
                if h not in myres and h != ip:
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        ...
                        if eventName == "NETBLOCK_OWNER":
                            ...
                            evt = GhostOsintEvent("CO_HOSTED_SITE", h.lower(), self.__name__, ipe)
                            self.notifyListeners(evt)
                        else:
                            evt = GhostOsintEvent("CO_HOSTED_SITE", h.lower(), self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if eventName == 'NETBLOCK_OWNER':
                pevent = GhostOsintEvent("IP_ADDRESS", ip, self.__name__, event)
            ...
            if self.opts.get('http_headers', True):
                ...
                if http_headers is not None:
                    e = GhostOsintEvent('WEBSERVER_HTTPHEADERS', json.dumps(http_headers), self.__name__, pevent)
                    e.actualSource = ip
                    self.notifyListeners(e)

# End of GO_hackertarget class
