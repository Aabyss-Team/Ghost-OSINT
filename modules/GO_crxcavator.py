...
class GO_crxcavator(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        domain_keyword = self.GhostOsint.domainKeyword(eventData, self.opts['_internettlds'])
        ...
        evt = GhostOsintEvent('RAW_RIR_DATA', json.dumps(results), self.__name__, event)
        ...
        for result in results:
            ...
            evt = GhostOsintEvent('RAW_RIR_DATA', json.dumps(extensions), self.__name__, event)
            self.notifyListeners(evt)
            ...
            for extension in extensions:
                ...
                if (
                    not self.getTarget().matches(self.GhostOsint.urlFQDN(privacy_policy), includeChildren=True, includeParents=True)
                    and not self.getTarget().matches(self.GhostOsint.urlFQDN(website), includeChildren=True, includeParents=True)
                    and not self.getTarget().matches(self.GhostOsint.urlFQDN(offered_by), includeChildren=True, includeParents=True)
                    and not self.getTarget().matches(self.GhostOsint.urlFQDN(support_site), includeChildren=True, includeParents=True)
                ):
                ...
                evt = GhostOsintEvent('APPSTORE_ENTRY', app_data, self.__name__, event)
                self.notifyListeners(evt)
        ...
        for url in set(urls):
            ...
            host = self.GhostOsint.urlFQDN(url)
        ...
        for location in set(locations):
            evt = GhostOsintEvent("PHYSICAL_ADDRESS", location, self.__name__, event)
            self.notifyListeners(evt)

# End of GO_crxcavator class
