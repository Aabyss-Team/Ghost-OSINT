...
# -------------------------------------------------------------------------------
# Name:        GO_spyse
# Purpose:     GhostOSINT plug-in to search Spyse API for IP address and
#              domain information.
#
...
class GO_spyse(GhostOsintPlugin):
    ...
    def queryIPPort(self, qry):
        ...
        Returns:
            dict: JSON formatted results
        """
        if not self.GhostOsint.validIP(qry):
    ...
    def queryDomainsOnIP(self, qry, currentOffset):
        ...
        if self.GhostOsint.validIP(qry):
            search_key = 'dns_a'
        elif self.GhostOsint.validIP6(qry):
            search_key = 'dns_aaaa'
        else:
        ...
        while True:
            ...
            if records:
                for record in records:
                    ...
                    if domain:
                        evt = GhostOsintEvent('RAW_RIR_DATA', str(record), self.__name__, event)
        ...
        for record in records:
            ...
            if cve_list:
                for cve in cve_list:
                    cve_id = cve.get('id')
                    if cve_id:
                        etype, cvetext = self.GhostOsint.cveInfo(cve_id)
                        evt = GhostOsintEvent(etype, cvetext, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if technologies:
                for tech in technologies:
                    ...
                    if software:
                        evt = GhostOsintEvent('SOFTWARE_USED', f"{software}", self.__name__, event)
                        self.notifyListeners(evt)
            ...
            for port_data in ports:
                ...
                evt = GhostOsintEvent('RAW_RIR_DATA', json.dumps(port_data), self.__name__, event)
                self.notifyListeners(evt)
                ...
                port_event = GhostOsintEvent('TCP_PORT_OPEN', ip_port, self.__name__, event)
                self.notifyListeners(port_event)

                banner = port_data.get('banner')
                if banner:
                    evt = GhostOsintEvent('TCP_PORT_OPEN_BANNER', json.dumps(banner), self.__name__, port_event)
                    self.notifyListeners(evt)

    def retrieve_domain_info(self, event):
        eventData = event.data
        if event.module == "GO_spyse" and (self.getTarget().targetValue in event.data):
            return
            ...
            for org in domain_item.get("organizations", []):
                ...
                if crunchbase:
                    if crunchbase.get("is_primary", False):
                        ...
                        if org_name:
                            evt = GhostOsintEvent('COMPANY_NAME', org_name, self.__name__, event)
                            self.notifyListeners(evt)
        ...
        if domain_dns:
            ...
            if domain_dns_a_records:
                for dns_A in domain_dns_a_records:
                    if dns_A:
                        evt = GhostOsintEvent('IP_ADDRESS', dns_A, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if domain_dns_aaaa_records:
                for dns_AAAA in domain_dns_aaaa_records:
                    if dns_AAAA:
                        evt = GhostOsintEvent('IPV6_ADDRESS', dns_AAAA, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if domain_dns_spf_records:
                for dns_spf in domain_dns_spf_records:
                    if dns_spf:
                        ...
                        if dns_spf_raw:
                            evt = GhostOsintEvent('DNS_SPF', dns_spf_raw, self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if domain_dns_txt_records:
                for dns_txt in domain_dns_txt_records:
                    if dns_txt:
                        evt = GhostOsintEvent('DNS_TEXT', dns_txt, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if domain_dns_ns_records:
                for dns_ns in domain_dns_ns_records:
                    if dns_ns:
                        evt = GhostOsintEvent('PROVIDER_DNS', dns_ns, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if domain_dns_mx_records:
                for dns_mx in domain_dns_mx_records:
                    if dns_mx:
                        evt = GhostOsintEvent('PROVIDER_MAIL', dns_mx, self.__name__, event)
                        self.notifyListeners(evt)
        ...
        if domain_technologies:
            for tech in domain_technologies:
                ...
                software = ' '.join(filter(None, [name, version]))
                if software:
                    evt = GhostOsintEvent('WEBSERVER_TECHNOLOGY', f"{software}", self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if domain_whois:
            ...
            if domain_whois_registrar:
                ...
                if registrar_name:
                    evt = GhostOsintEvent('DOMAIN_REGISTRAR', registrar_name, self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if domain_http_extract:
            ...
            if http_status_code:
                evt = GhostOsintEvent('HTTP_CODE', str(http_status_code), self.__name__, event)
                self.notifyListeners(evt)

            http_headers = domain_http_extract.get("http_headers")
            if http_headers:
                evt = GhostOsintEvent('WEBSERVER_HTTPHEADERS', str(http_headers), self.__name__, event)
                self.notifyListeners(evt)
            ...
            if domain_emails:
                for email in domain_emails:
                    if self.GhostOsint.validEmail(email):
                        evt = GhostOsintEvent('EMAILADDR', email, self.__name__, event)
                        self.notifyListeners(evt)
        ...
        if domain_cert_summary:
            ...
            if domain_cert_summary_subject:
                ...
                if cert_issued:
                    evt = GhostOsintEvent('SSL_CERTIFICATE_ISSUED', cert_issued, self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if domain_cert_summary_issuer:
                ...
                if cert_issuer:
                    evt = GhostOsintEvent('SSL_CERTIFICATE_ISSUER', cert_issuer, self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if domain_trackers:
            ...
            if google_analytics_key:
                evt = GhostOsintEvent("WEB_ANALYTICS_ID", f"Google Analytics: {google_analytics_key}", self.__name__, event)
                self.notifyListeners(evt)
