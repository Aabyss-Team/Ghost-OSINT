...
# -------------------------------------------------------------------------------
# Name:         GO_tldsearch
# Purpose:      GhostOSINT plug-in for identifying the existence of this target
#               on other TLDs.
#
...
class GO_tldsearch(GhostOsintPlugin):
    ...
    def tryTld(self, target, tld):
        ...
        if self.opts['skipwildcards'] and self.GhostOsint.checkDnsWildcard(tld):
            return

        try:
            if not self.GhostOsint.resolveHost(target) and not self.GhostOsint.resolveHost6(target):
                with self.lock:
    ...
    def tryTldWrapper(self, tldList, sourceEvent):
        ...
        for pair in tldList:
            (domain, tld) = pair
            tn = 'thread_GO_tldsearch_' + str(random.SystemRandom().randint(0, 999999999))
        ...
        while running:
            ...
            for rt in threading.enumerate():
                if rt.name.startswith("thread_GO_tldsearch_"):
                    found = True
    ...
    def sendEvent(self, source, result):
        ...
        if self.opts['activeonly']:
            ...
            pageContent = self.GhostOsint.fetchUrl('http://' + result,
                                           timeout=self.opts['_fetchtimeout'],
                                           useragent=self.opts['_useragent'],
                                           noLog=True,
                                           verify=False)
            if pageContent['content'] is not None:
                evt = GhostOsintEvent("SIMILARDOMAIN", result, self.__name__, source)
