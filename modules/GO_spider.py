...
# -------------------------------------------------------------------------------
# Name:         GO_spider
# Purpose:      GhostOSINT plug-in for spidering sites and returning meta data
...
from ghostosint import GhostOsintEvent, GhostOsintHelp, GhostOsintPlugin
...
class GO_spider(GhostOsintPlugin):
    ...
    def processUrl(self, url):
        site = self.GhostOsint.urlFQDN(url)
        cookies = None
        ...
        fetched = self.GhostOsint.fetchUrl(
            url,
            False,
            cookies,
        ...
        links = self.GhostOsint.parseLinks(url, fetched['content'],
        ...
        for link in links:
            if not self.opts['reportduplicates']:
                if link in self.urlEvents:
                    continue
            # Supply the GhostOsintEvent of the parent URL as the parent
    ...
    def cleanLinks(self, links):
        ...
        for link in links:
            linkBase = self.GhostOsint.urlBaseUrl(link)
            linkFQDN = self.GhostOsint.urlFQDN(link)
    ...
    def linkNotify(self, url, parentEvent=None):
        if self.getTarget().matches(self.GhostOsint.urlFQDN(url)):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "INTERNET_NAME":
            for prefix in self.opts['start']:
                res = self.GhostOsint.fetchUrl(prefix + eventData, timeout=self.opts['_fetchtimeout'],
                                       useragent=self.opts['_useragent'],
                                       verify=False)
                if res['content'] is not None:
                    spiderTarget = prefix + eventData
                    evt = GhostOsintEvent("LINKED_URL_INTERNAL", spiderTarget,
                                          self.__name__, event)
    ...
    def spiderFrom(self, startingPoint):
        ...
        levelsTraversed = 0
        nextLinks = dict()
        targetBase = self.GhostOsint.urlBaseUrl(startingPoint)

        # Are we respecting robots.txt?
        if self.opts['robotsonly'] and targetBase not in self.robotsRules:
            robotsTxt = self.GhostOsint.fetchUrl(targetBase + '/robots.txt',
                                         timeout=self.opts['_fetchtimeout'],
                                         useragent=self.opts['_useragent'],
                                         verify=False)
            if robotsTxt['content'] is not None:
                self.debug('robots.txt contents: ' + robotsTxt['content'])
                self.robotsRules[targetBase] = GhostOsintHelp.parseRobotsTxt(robotsTxt['content'])
