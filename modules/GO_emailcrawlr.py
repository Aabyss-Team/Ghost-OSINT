...
class GO_emailcrawlr(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled GO_emailcrawlr but did not set an API key!")
            self.errorState = True
            return
            ...
            for res in emails:
                ...
                if name:
                    ...
                    if full_name and len(full_name) > 3:
                        evt = GhostOsintEvent("RAW_RIR_DATA", f"Possible full name: {full_name}",
                                              self.__name__, event)
                        self.notifyListeners(evt)
                ...
                if phone_numbers:
                    for number in phone_numbers:
                        if number:
                            evt = GhostOsintEvent("PHONE_NUMBER", number, self.__name__, event)
                            self.notifyListeners(evt)
                ...
                if location:
                    ...
                    if loc:
                        evt = GhostOsintEvent("GEOINFO", loc, self.__name__, event)
                        self.notifyListeners(evt)

# End of GO_emailcrawlr class
