...
class GO_riskiq(GhostOsintPlugin):
        ...
        if eventName == 'EMAILADDR':
            ...
            for r in ret:
                if not eventData.endswith("@" + r['domain']):
                    if self.GhostOsint.validIP(r['domain']):
                        t = "NETBLOCK_OWNER"
                    else:
                        t = "AFFILIATE_INTERNET_NAME"
                    e = GhostOsintEvent(t, r['domain'], self.__name__, event)
                    self.notifyListeners(e)

                    if t == "AFFILIATE_INTERNET_NAME" and self.GhostOsint.isDomain(r['domain'], self.opts['_internettlds']):
                        evt = GhostOsintEvent("AFFILIATE_DOMAIN_NAME", r['domain'], self.__name__, event)
            ...
            for co in cohosts:
                ...
                if not self.opts['cohostsamedomain']:
                    if self.getTarget().matches(co, includeParents=True):
                        if self.GhostOsint.resolveHost(co) or self.GhostOsint.resolveHost6(co):
                            e = GhostOsintEvent("INTERNET_NAME", co, self.__name__, event)
                        else:
                            e = GhostOsintEvent("INTERNET_NAME_UNRESOLVED", co, self.__name__, event)
                        self.notifyListeners(e)

                        if self.GhostOsint.isDomain(co, self.opts['_internettlds']):
                            e = GhostOsintEvent("DOMAIN_NAME", co, self.__name__, event)
                            self.notifyListeners(e)
                        continue
