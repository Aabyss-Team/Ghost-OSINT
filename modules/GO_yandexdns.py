...
class GO_yandexdns(GhostOsintPlugin):
    ...
    def handleEvent(self, event):
        ...
        for result in res:
            ...
            evt = GhostOsintEvent(blacklist_type, f"{self.checks[k]} [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)

            if k == '213.180.193.250':
                evt = GhostOsintEvent(malicious_type, f"{self.checks[k]} [{eventData}]", self.__name__, event)
                self.notifyListeners(evt)

# End of GO_yandexdns class
