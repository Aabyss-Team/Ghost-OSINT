# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         GO_portscan_tcp
# Purpose:      GhostOSINT plug-in for performing a basic TCP port scan of IP
...
class GO_portscan_tcp(GhostOsintPlugin):
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        if self.opts['ports'][0].startswith("http://") or \
            ...
            file_ports = self.GhostOsint.optValueToData(self.opts['ports'][0])
            if file_ports:
    ...
    def tryPort(self, ip, port):
        ...
        try:
            sock = self.GhostOsint.safeSocket(ip, port, self.opts['timeout'])
            with self.lock:
    ...
    def tryPortWrapper(self, ip, portList):
        ...
        while i < len(portList):
            ...
            t.append(threading.Thread(name=f"GO_portscan_tcp_{port}", target=self.tryPort, args=(ip, port)))
            t[i].start()
            i += 1
        ...
        while running:
            ...
            for rt in threading.enumerate():
                if rt.name.startswith("GO_portscan_tcp_"):
                    found = True
    ...
    def sendEvent(self, resArray, srcEvent):
        for cp in resArray:
            ...
            evt = GhostOsintEvent("TCP_PORT_OPEN", cp, self.__name__, srcEvent)
            self.notifyListeners(evt)

            if resArray[cp] is not True:
                banner = str(resArray[cp], 'utf-8', errors='replace')
                bevt = GhostOsintEvent("TCP_PORT_OPEN_BANNER", banner, self.__name__, evt)
