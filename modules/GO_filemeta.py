    ...
    def handleEvent(self, event):
        ...
        for fileExt in self.opts['fileexts']:
            ...
            if "." + fileExt.lower() in eventData.lower():
                ...
                # typically large.
                ret = self.GhostOsint.fetchUrl(eventData, timeout=self.opts['timeout'],
                ...
                if meta is not None and data is not None:
                    evt = GhostOsintEvent("RAW_FILE_META_DATA", meta,
                                          self.__name__, event)
                    ...
                    for v in val:
                        if v and not isinstance(v, PyPDF2.generic.NullObject):
                            ...
                            evt = GhostOsintEvent("SOFTWARE_USED", v, self.__name__, event)
                            self.notifyListeners(evt)
