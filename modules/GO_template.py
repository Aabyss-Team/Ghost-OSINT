...
class GO_template(GhostOsintPlugin):
    ...
    meta = {
        ...
        'summary': "This is an example module to help developers create their own GhostOSINT modules.",
        ...
        # that purley parse data from other modules (e.g. GO_email), this may be omitted.
        'dataSource': {
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        # instead since on GhostOSINT HX, different mechanisms are used to persist
        ...
        # utilised in GhostOSINT HX and not the open source version.
    ...
    # For a list of all events, check ghostosint/db.py.
    ...
    def query(self, qry):
        ...
        # from global config), and the user agent is GhostOSINT so that the
        ...
        # - fatal(message) if it's a horrible thing and should kill GhostOSINT completely
    ...
    def handleEvent(self, event):
        # The three most used fields in GhostOsintEvent are:
        # event.eventType - the event type, e.g. INTERNET_NAME, IP_ADDRESS, etc.
        # event.module - the name of the module that generated the event, e.g. GO_dnsresolve
        ...
        for addr in qrylist:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                # to process and is a fundamental part of the GhostOSINT architecture.
                ...
                # event of this event. This enables GhostOSINT to link events so users
            ...
            # itself! GO_email is a good example, since it is purely looking
            ...
            os = rec.get('os')
            if os:
                evt = GhostOsintEvent("OPERATING_SYSTEM", f"{os} ({addr})", self.__name__, pevent)
                self.notifyListeners(evt)

# End of GO_template class
