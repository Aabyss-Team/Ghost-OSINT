...
class GO_metadefender(GhostOsintPlugin):
    ...
    def queryDomain(self, qry):
        ...
        res = self.GhostOsint.fetchUrl('https://api.metadefender.com/v4/domain/' + qry,
                               headers=headers,
                               timeout=15,
    ...
    def queryIp(self, qry):
        ...
        res = self.GhostOsint.fetchUrl('https://api.metadefender.com/v4/ip/' + qry,
                               headers=headers,
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled GO_metadefender but did not set an API key!")
            self.errorState = True
            return
        ...
        if eventName == 'IP_ADDRESS':
            ...
            for m in sources:
                ...
                evt = GhostOsintEvent('MALICIOUS_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = GhostOsintEvent('BLACKLISTED_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName == 'INTERNET_NAME':
            ...
            for m in sources:
                ...
                if m['assessment'] == "trustworthy":
                    continue
                provider = m.get('provider')
                evt = GhostOsintEvent('MALICIOUS_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = GhostOsintEvent('BLACKLISTED_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)

# End of GO_metadefender class
