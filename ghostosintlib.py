...
class GhostOSINT:
    ...
    Attributes:
        ...
        scanId (str): scan ID this instance of GhostOSINT is being used in
    ...
    def scanId(self, scanId: str) -> str:
        """Set the scan ID this instance of GhostOSINT is being used in.
    ...
    def socksProxy(self, socksProxy: str) -> str:
        ...
        modules. ghostosintscan will call this to update the socket reference
        to the SOCKS one.
    ...
    def optValueToData(self, val: str) -> str:
        ...
        if not isinstance(val, str):
            self.error(f"无效选项值 {val}")
            return None

        if val.startswith('@'):
            fname = val.split('@')[1]
            self.info(f"正在加载配置数据: {fname}")

            try:
                with open(fname, "r") as f:
                    return f.read()
            except Exception as e:
                self.error(f"无法打开选项文件, {fname}: {e}")
                return None

        if val.lower().startswith('http://') or val.lower().startswith('https://'):
            try:
                self.info(f"正在下载配置数据: {val}")
                session = self.getSession()
                res = session.get(val)

                return res.content.decode('utf-8')
            except BaseException as e:
                self.error(f"无法打开选项URL, {val}: {e}")
                return None
    ...
    def cachePut(self, label: str, data: str) -> None:
        ...
        cacheFile = GhostOsintHelp.cachePath() + "/" + pathLabel
    ...
    def configSerialize(self, opts: dict, filterSystem: bool = True):
        ...
        Args:
            opts (dict): Dictionary of GhostOSINT configuration options
    ...
    def configUnserialize(self, opts: dict, referencePoint: dict, filterSystem: bool = True):
        ...
        Args:
            opts (dict): GhostOSINT configuration options
    ...
    def validHost(self, hostname: str, tldList: str) -> bool:
        ...
        gox = ps.privatesuffix(hostname)
        return gox is not None
    ...
    def dictwords(self) -> list:
        ...
        for d in dicts:
            try:
                with io.open(f"{self.myPath()}/ghostosint/dicts/ispell/{d}.dict", 'r', encoding='utf8', errors='ignore') as wdct:
                    dlines = wdct.readlines()
            except BaseException as e:
                self.debug(f"无法读取字典: {e}")
                continue
    ...
    def dictnames(self) -> list:
        ...
        for d in dicts:
            try:
                wdct = open(f"{self.myPath()}/ghostosint/dicts/ispell/{d}.dict", 'r')
                dlines = wdct.readlines()
                wdct.close()
            except BaseException as e:
                self.debug("无法读取字典: " + str(e))
                continue
    ...
    def resolveHost(self, host: str) -> list:
        ...
        try:
            ...
            self.debug(f"无法解析主机: {host} ({e})")
            return addrs

        if not addrs:
            self.debug(f"无法解析主机: {host}")
            return addrs

        self.debug(f"解决 {host} IPv4 主机: {addrs}")
    ...
    def resolveIP(self, ipaddr: str) -> list:
        ...
        if not self.validIP(ipaddr) and not self.validIP6(ipaddr):
            self.error(f"无法反向解析 {ipaddr} (IP地址无效)")
            return list()

        self.debug(f"正在执行反向解析 {ipaddr}")

        try:
            addrs = self.normalizeDNS(socket.gethostbyaddr(ipaddr))
        except BaseException as e:
            self.debug(f"无法反向解析IP地址: {ipaddr} ({e})")
            return list()

        if not addrs:
            self.debug(f"无法反向解析IP地址: {ipaddr}")
            return list()

        self.debug(f"解决反向 {ipaddr} 为: {addrs}")
    ...
    def resolveHost6(self, hostname: str) -> list:
        ...
        """
        if not hostname:
            self.error(f"无法解析主机: {hostname} (无效主机)")
            return list()
        ...
        try:
            ...
            self.debug(f"无法解析主机: {hostname} ({e})")
            return addrs

        if not addrs:
            self.debug(f"无法解析主机: {hostname}")
            return addrs

        self.debug(f"解决 {hostname} IPv6 主机: {addrs}")
    ...
    def validateIP(self, host: str, ip: str) -> bool:
        ...
        if self.validIP(ip):
            ...
            self.error(f"无法验证主机名 {host} 解决到 {ip} (IP地址无效)")
            return False
    ...
    def parseRobotsTxt(self, robotsTxtData: str) -> list:
        ...
        for line in robotsTxtData.splitlines():
            if line.lower().startswith('disallow:'):
                ...
                if m:
                    self.debug('robots.txt 不允许查看: ' + m.group(1))
    ...
    def parseLinks(self, url: str, data: str, domains: list) -> list:
        ...
        try:
            ...
            self.error("使用BeautifulSoup解析时报错: " + str(e))
            return returnLinks
        ...
        for link in urlsRel:
            ...
            if link[len(link) - 1] == '.' or link[0] == '+' or 'javascript:' in linkl or '()' in link:
                self.debug('不太可能的链接: ' + link)
                continue

            # Filter in-page links
            if re.match('.*#.[^/]+', link):
                self.debug('页面内链接: ' + link)
                continue

            # Ignore mail links
            if 'mailto:' in linkl:
                self.debug("忽略邮件链接: " + link)
                continue
    ...
    def fetchUrl(
        ...
        try:
            ...
            self.debug(f"无法分析URL地址: {url}")
            return None

        if parsed_url.scheme != 'http' and parsed_url.scheme != 'https':
            self.debug(f"URL无效: {url}")
            return None
        ...
        try:
            ...
            if sizeLimit and len(res.content) > sizeLimit:
                self.debug(f"内容超出大小限制 ({sizeLimit}), 所以不返回数据只返回HEAD包头")
            ...
            if refresh_header:
                try:
                    ...
                    self.debug(f"刷新HEAD包头找到 '{refresh_header}' , 但不可解析: {e}")
                    return result

                self.debug(f"刷新HEAD包头找到 '{refresh_header}' , 正在重定向到 {self.removeUrlCreds(newurl)}")
            ...
            if fatal:
                try:
                    ...
                    self.fatal(f"无法获取URL地址 ({res.status_code}) / {res.content})")
    ...
    def cveInfo(self, cveId: str, sources="circl,nist") -> (str, str):
        ...
        for source in sources:
            ...
            try:
                ...
                self.debug(f"无法解析CVE的异常响应 {source.upper()}: {e}")
                continue
    ...
    def googleIterate(self, searchString: str, opts: dict = None) -> dict:
        ...
        if response['code'] != '200':
            self.error("无法从Google API 获取有效响应")
            return None

        try:
            response_json = json.loads(response['content'])
        except ValueError:
            self.error("Google API 响应中的'content'不包含有效的JSON.")
            return None
    ...
    def bingIterate(self, searchString: str, opts: dict = {}) -> dict:
        ...
        try:
            ...
            self.error("Bing API 响应中的'content'不包含有效的JSON.")
            return None
